...
interface PortfolioBrainEvolutionSyncReport {
  version: string;
  status: string;
...
function getRiskClass(value: number) {
  if (value < 0) {
...
export default function PortfolioBrainEvolutionDashboardPanel() {
  const [report, setReport] =
    useState<PortfolioBrainEvolutionSyncReport | null>(null);
  ...
  useEffect(() => {
    async function loadData() {
      ...
      try {
        const response =
          await fetch(
            "/api/portfolio-brain-evolution-sync",
            {
              cache: "no-store",
            }
          );
        ...
        console.error(
          "Portfolio Brain Evolution Sync Error",
          error
        );
  ...
  const groupedDecisions =
    useMemo(() => {
      const groups: Record<EvolutionDecisionStatus, EvolutionDecision[]> = {
        PROTECTED: [],
        ACTIVE: [],
        REDUCED: [],
        ARCHIVED: [],
    ...
    <section className="rounded-3xl border border-slate-800 bg-slate-950/80 p-6 shadow-2xl shadow-cyan-950/20">
      ...
      <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
        ...
        <div>
          ...
          <p className="mt-2 max-w-3xl text-sm text-slate-400">
            Synchronisiert Evolution Governance mit Portfolio Brain. Portfolio
            Bias, Risk Adjustment und Species-Status werden sichtbar für spätere
            Allocation- und Risk-Entscheidungen.
          </p>
        </div>
      ...
      {loading && (
        <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-4 text-sm text-slate-400">
          Lade Portfolio Brain Evolution Sync...
        </div>
      )}
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            ...
            <div className="rounded-2xl border border-slate-800 bg-slate-900/80 p-5">
              ...
              <p className={`mt-3 text-4xl font-black ${getRiskClass(
                report.portfolioRiskAdjustment
              )}`}>
                {report.portfolioRiskAdjustment}
              </p>

              <p className="mt-2 text-sm text-slate-400">
                Negative Werte bedeuten vorsichtigere Portfolio-Risk-Steuerung.
              </p>
          ...
          <div className="grid gap-4 lg:grid-cols-4">
            ...
            {(["PROTECTED", "ACTIVE", "REDUCED", "ARCHIVED"] as EvolutionDecisionStatus[]).map(
              (status) => (
                <div
                  key={status}
                    ...
                    {groupedDecisions[status].map((decision) => (
                      ...
                      >
                        ...
                        <p className="mt-1 text-xs text-slate-400">
                          Governance Score {decision.governanceScore}
                        </p>
                      </div>
                    ))}
      ...
      {!loading && !report && (
        <div className="rounded-2xl border border-red-800/60 bg-red-950/30 p-4 text-sm text-red-300">
          Portfolio Brain Evolution Sync Daten konnten nicht geladen werden.
        </div>
      )}
